...
)
{
  ...
  for (size_t i = 0; i < 16; i++) {
    ...
    tag[i] = (state[off] << 4) | (state[off ^ 1] & photon::LS4B);
  }
  ...
  for (size_t i = 0; i < 16; i++) {
    ...
    tag[16 ^ i] = (state[off] << 4) | (state[off ^ 1] & photon::LS4B);
  }
}
...
)
{
  ...
  for (size_t i = 0; i < full_blk; i++) {
    ...
    const uint32_t y = (static_cast<uint32_t>(state[0] & photon::LS4B) << 28) |
                       (static_cast<uint32_t>(state[1] & photon::LS4B) << 24) |
                       (static_cast<uint32_t>(state[2] & photon::LS4B) << 20) |
                       (static_cast<uint32_t>(state[3] & photon::LS4B) << 16) |
                       (static_cast<uint32_t>(state[4] & photon::LS4B) << 12) |
                       (static_cast<uint32_t>(state[5] & photon::LS4B) << 8) |
                       (static_cast<uint32_t>(state[6] & photon::LS4B) << 4) |
                       (static_cast<uint32_t>(state[7] & photon::LS4B) << 0);

    const uint32_t w = y ^ d;

    state[0] = static_cast<uint8_t>(w >> 28) & photon::LS4B;
    state[1] = static_cast<uint8_t>(w >> 24) & photon::LS4B;
    state[2] = static_cast<uint8_t>(w >> 20) & photon::LS4B;
    state[3] = static_cast<uint8_t>(w >> 16) & photon::LS4B;
    state[4] = static_cast<uint8_t>(w >> 12) & photon::LS4B;
    state[5] = static_cast<uint8_t>(w >> 8) & photon::LS4B;
    state[6] = static_cast<uint8_t>(w >> 4) & photon::LS4B;
    state[7] = static_cast<uint8_t>(w >> 0) & photon::LS4B;
  }

  if (rm_bytes > 0ul) {
    const size_t off = full_blk << 2;
    uint32_t d = 0b10000000u << ((3 - rm_bytes) << 3);
  ...
  if (mlen == 0ul) {
    ...
    if (mlen <= 16ul) {
      for (size_t i = 0; i < mlen; i++) {
        ...
        state[off] = msg[i] >> 4;
        state[off ^ 1] = msg[i] & photon::LS4B;
      }

      constexpr uint8_t br0[2] = { 0, 0b00001000 };
